...
class TopicCopy extends Form {
  ...
  componentDidMount() {
    ...
    this.setState(
      { clusterId, topicId, formData: { clusterListView: clusterId, lastMessagesNr: '' } },
      () => {
  ...
  getTopics = clusterId => {
    this.getApi(uriTopicsName(clusterId))
      .then(res => {
        this.setState({ topics: transformStringArrayToViewOptions(res.data), loading: false });
      })
      .catch(err => {
        console.error('Error:', err);
      });
  ...
  handleOnChangeTopic = () => {
    ...
    this.getApi(uriTopicsInfo(formData.clusterListView, formData.topicListView))
      .then(res => {
        this.setState({ selectedTopic: res.data, loading: false });
      })
      .catch(err => {
        console.error('Error:', err);
      });
  ...
  createSubmitBody = (formData, checked) => {
    ...
    Object.keys(formData)
      .filter(value => value.startsWith('partition'))
      .forEach(name => {
  ...
  async doSubmit() {
    ...
    toast.success(
      `Copied ${result.data.records} records to topic '${formData.topicListView}' successfully.`
    );
  }
